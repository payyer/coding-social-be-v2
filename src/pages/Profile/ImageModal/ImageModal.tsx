...
export const ImageModal = () => {
  ...
  const increaseImage = (
    ...
    if (
      data?.metadata &&
      imageModal.indexClickImage > data?.metadata.length - 2
    ) {
      dispatch(
        setImageModal({
          isOpen: true,
    ...
    if (imageModal.indexClickImage > 0) {
      ...
      if (data?.metadata)
        dispatch(
          setImageModal({
            isOpen: true,
            indexClickImage: data?.metadata.length - 1,
          })
        );
    }
  };
        ...
        >
          <div onClick={(e) => e.stopPropagation()} className="w-[400px] flex">
            ...
            <div className="flex-1 h-full">
              {data?.metadata[imageModal.indexClickImage] && (
                <img
                  src={data?.metadata[imageModal.indexClickImage].public_id}
                  alt=""
